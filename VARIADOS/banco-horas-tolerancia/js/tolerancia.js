...
// =============================================================
// MODAL: EXPLICAÇÃO (Tolerância 15 min x Banco de Horas)
// =============================================================
const BH_INFO_URL = "banco-horas-tolerancia/index.html";

window.openBhInfoModal = function () {
  const modal = document.getElementById("bhInfoModal");
  const frame = document.getElementById("bhInfoFrame");
  if (!modal || !frame) return;

  // Carrega apenas quando abrir (mantém leve)
  if (!frame.src) frame.src = BH_INFO_URL;

  modal.classList.add("active");
  modal.setAttribute("aria-hidden", "false");
};

window.closeBhInfoModal = function () {
  const modal = document.getElementById("bhInfoModal");
  if (!modal) return;
  modal.classList.remove("active");
  modal.setAttribute("aria-hidden", "true");
...
window.openBhInfoPage = function () {
  window.open(BH_INFO_URL, "_blank", "noopener,noreferrer");
};

function setupBhInfoModalUX() {
  const modal = document.getElementById("bhInfoModal");
  if (!modal) return;

  // Fechar ao clicar fora
  modal.addEventListener("click", (e) => {
    if (e.target === modal) closeBhInfoModal();
  });

  // Fechar no ESC
  document.addEventListener("keydown", (e) => {
    if (e.key === "Escape" && modal.classList.contains("active")) closeBhInfoModal();
  });
}
